...
if len(sys.argv)!=3:
    raise SystemExit('usage: install_ui_feedback_v2.py INPUT OUTPUT')
...
status_new="""  function setGatherStatus(show,message){
  ...
  window.gmSetPrintStatus=function(show,message){
    var st=ensurePrintStatus();
    st.textContent=message||'The Greenman is getting your pages ready to print…';
    st.classList.toggle('show',!!show);
  };
  function setGatherButtonsDisabled(flag){"""
replace_once(esc(status_old),esc(status_new),'Gather dialog / global print status')
...
render_new="""function renderPrint(pages,kind){
 if(!pages.length){busy=false;window.gmSetPrintStatus(false);alert('No completed print pages were ready.');return}"""
...
print_call_new=""" setTimeout(function(){try{if(typeof window.gmNumerologyLines==='function')window.gmNumerologyLines(root,sp)}catch(e){}qa('.true-page',d).forEach(fitPage);busy=false;window.gmSetPrintStatus(false);try{printFrame.contentWindow.document.title=title;printFrame.contentWindow.focus();printFrame.contentWindow.print()}catch(e){console.error(e)}},900);"""
...
pack_new="""function buildPack(){
 if(busy)return;busy=true;window.gmSetPrintStatus(true,'The Greenman is getting your A4 pack ready to print…');var native=window.print;window.print=function(){};
 try{var old=q('#gm-v40-print-stack');if(old)old.remove();window.gmV40Print()}catch(e){window.print=native;busy=false;window.gmSetPrintStatus(false);console.error(e);return}"""
...
page_new="""function buildPage(){if(busy)return;busy=true;window.gmSetPrintStatus(true,'The Greenman is getting your A4 page ready to print…');var f=window.GM_CURRENT_FILE||'Page';"""
...
lite_new="""function printLiteThree(){
 ...
 var p1=document.getElementById('print-pack-summary-page-1'),p2=document.getElementById('print-pack-summary-page-2');
 if(!p1||!p2){alert('Open both Summary pages before printing the Lite Pack.');return;}
 window.gmSetPrintStatus(true,'The Greenman is getting your Lite pack ready to print…');"""
...
if esc(lite_call_old) not in s:
    lite_call_old="""  try{f.contentWindow.focus();f.contentWindow.print();}finally{setTimeout(function(){f.remove();},1800);}"""
lite_call_new="""  window.gmSetPrintStatus(false);
