...
# 2.7.8 retains the repaired 2.7.6 print bridge + hardening: harden the frozen print copy, use canonical Tumbler God/Goddess SVGs,
...
cp "$JAVA_FILE" "$RUNNER_TEMP/MainActivity_2_7_8_BOS_SNAPSHOT_SCALE_RECOVERY.java"
sha256sum "$JAVA_FILE" > "$RUNNER_TEMP/GREENMAN_2_7_8_JAVA_SHA256.txt"
...
sed -i -E 's/versionCode[[:space:]]+[0-9]+/versionCode 26/' "$PROJECT_DIR/app/build.gradle"
sed -i -E 's/versionName[[:space:]]+"[^"]+"/versionName "2.7.8-bos-snapshot-scale-recovery"/' "$PROJECT_DIR/app/build.gradle"
...
grep -q "package: name='com.greenman.hedgewitchery' versionCode='26'" "$RUNNER_TEMP/GREENMAN_27_BADGING.txt"
...
cp "$APK_FILE" "$RUNNER_TEMP/GREENMAN_HEDGEWITCHERY_2.7.8_BOS_SNAPSHOT_SCALE_RECOVERY.apk"
echo "APK_FILE=$RUNNER_TEMP/GREENMAN_HEDGEWITCHERY_2.7.8_BOS_SNAPSHOT_SCALE_RECOVERY.apk" >> "$GITHUB_ENV"
