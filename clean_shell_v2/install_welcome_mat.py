...
# Samsung WebView can overlay the keyboard without shrinking VisualViewport. Do not
# depend on keyboard-height measurement. As soon as the Gate input is focused, pin the
# complete card near the top of the app so its buttons remain above the keyboard.
...
#gmGate.gm-keyboard-open{align-items:flex-start!important;justify-content:center!important;padding:12px 18px 10px!important;overflow:visible!important}
#gmGate.gm-keyboard-open .gate-card{position:absolute!important;top:18px!important;left:50%!important;transform:translateX(-50%)!important;width:min(420px,calc(100% - 36px))!important;margin:0!important;max-height:none!important;overflow:visible!important}
#gmGate.gm-keyboard-open .gate-row{position:relative;z-index:2}
@media(max-height:700px){#gmGate.gm-keyboard-open .gate-card{top:8px!important;padding:14px!important}.gm-keyboard-open .gate-title{margin-bottom:5px!important}.gm-keyboard-open .gate-text{margin-bottom:8px!important}.gm-keyboard-open .gate-row{margin-top:7px!important}.gm-keyboard-open .gate-msg{margin-top:5px!important;min-height:14px!important}}'''
...
gate_js_new = '''function gmSyncGateKeyboard(){
  ...
  g.classList.toggle('gm-keyboard-open',inputActive);
}
...
document.getElementById('gateInput').addEventListener('focus',function(){gmSyncGateKeyboard();setTimeout(gmSyncGateKeyboard,80);});
document.getElementById('gateInput').addEventListener('blur',function(){setTimeout(gmSyncGateKeyboard,220);});
document.getElementById('gmOpenFullFloat').addEventListener('click', ()=>openGate(false));'''
...
for required in (
    '#gmGate.gm-keyboard-open .gate-card{position:absolute!important;top:18px!important',
    'function gmSyncGateKeyboard()',
    "g.classList.toggle('gm-keyboard-open',inputActive)",
    "document.getElementById('gateInput').addEventListener('focus'"
):
    if required not in s:
        raise SystemExit(f'missing Greenman Gate keyboard change: {required}')

out.write_text(s, encoding='utf-8')
print('Installed welcoming mat plus top-pinned keyboard-safe Greenman Gate')
